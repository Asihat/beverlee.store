...
//        if (md5($packet_id_array.$order_id) == $token){
//            if (count($payment) > 0) {
//                return 'false';
//            } else {
...
class CheckDataController extends Controller {
                ...
                foreach ($total_products as $product) {
                    foreach ($goods as $good) {
                        ...
                        if ($product->id == $good->product_id) {
                            if (!($product->amount > 0) AND $good->total_amount > 0) {
                                $good->total_amount--;
    ...
    public function test() {
        $sql = <<<SQL
    SELECT * FROM `payments` WHERE TIMESTAMPDIFF(MINUTE,created_at,NOW()) > 30
SQL;
